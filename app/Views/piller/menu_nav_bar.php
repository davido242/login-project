<?php echo view('piller/notification_header');?>
<main>
  <!-- Nav-bar Section -->
  <nav class="chat-nav mystyle" id="myDIV">
    <div class="notification-box">
      <div class="image-circle">
        <!-- empty circle -->
      </div>
      <div class="caption-holder">
        <p>John Agbaeze</p>
        <p>Enugu, NG</p>
      </div>
    </div>
    <ul>
      <li><span><i class="fa fa-bell note-bars"></i></span><a href="notificaiton">Notification</a></li>
      <li><span><i class="fa fa-comment note-bars"></i></span><a href="chat">Chat</a></li>
      <li><span><i class="fa fa-user-circle note-bars"></i></span><a href="#">Profile</a></li>
      <li><span><i class="fa fa-share-square note-bars"></i></span><a href="#">Subscription</a></li>
      <li><span><i class="fa fa-power-off note-bars"></i></span><a href="#">Logout</a></li>
    </ul>
  </nav>


  <?php echo view('piller/notification_main_nav');?>

  <!-- notificaiton section -->



  <!-- end of notification section -->
</main>